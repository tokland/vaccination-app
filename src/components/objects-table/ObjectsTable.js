...
class ObjectsTable extends React.Component {
    ...
    async getDataSets({clearPage = true} = {}) {
        ...
        const dataRows = dataSetsCollection.toArray()
            .map(dr => _.merge(dr, {selected: false, lastUpdatedHuman: formatDateLong(dr.lastUpdated)}));
    ...
    _toggleShowOnlyUserCampaigns = (ev) => {
        this.setState({showOnlyUserCampaigns: ev.target.checked, isLoading: true}, this.getDataSets);
    }
    ...
    _getPaginationProps() {
        ...
        return {
            hasNextPage: () =>
                Boolean(pager.hasNextPage) && pager.hasNextPage(),
            hasPreviousPage: () =>
                Boolean(pager.hasPreviousPage) && pager.hasPreviousPage(),
            onNextPageClick: () => {
                this.setState({
                    isLoading: true,
                    page: pager.page + 1
    ...
    isContextActionAllowed = (...args) => {
        this.actions.isContextActionAllowed(this.props.d2, ...args);
    }

    render() {
        const { dataRows, showOnlyUserCampaigns } = this.state;
        const paginationProps = this._getPaginationProps();
        const rows = dataRows.map(dr => fp.merge(dr, {selected:
            (<SimpleCheckBox onClick={ev => this.onSelectToggle(ev, dr)} checked={dr.selected} />)}));
        const selectedHeaderChecked = !_.isEmpty(dataRows) &&
            dataRows.every(row => row.selected);
        ...
        const columns = [
            ...
            {name: 'lastUpdated', text: i18n.t('Last updated'), sortable: true, value: "lastUpdatedHuman"},
        ];

        const activeRows = _(rows).keyBy("id")
            .at(dataRows.filter(dr => dr.selected).map(dr => dr.id)).value();
                ...
                <div style={styles.listDetailsWrap}>
                    <div style={styles.dataTableWrap}>
                        <MultipleDataTable
                            ...
                            />
                        {dataRows.length > 0 || this.state.isLoading
                            ? null
